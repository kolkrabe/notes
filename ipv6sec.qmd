...
-   Protects against weakness in the ICMPv6 Packets which are flooded (not broadcast) to
...
-   Makes use of 2 High Level Features
    -   <div>

        1.  IPv6 Snooping Policy (Container that enables most other features)
        2.  IPv6 Binding Table: Database of connected IPv6 Neighbours, gleaned from various FHS features.
...
-   **RA Guard (Router Advertisement)**
    ...
    -   Can allow/Block based on
        -   Source Link Local Address
        -   GLobal prefixes advertised within an RA
        -   Is the "Managed Config" flag on or off?
        -   Is the "Other Config" flag on or off
        -   A minimum hop-limit advertised within the RA
    -   2 modes ( Host & Router)
...
-   **IPv6 Snooping**
    ...
    -   **Inspect** gleans addresses and validates messages for consistency & conformance and enforces address ownership. Allows Router Advertisements & DHCP Server Messages.
    -   **Guard** gleans *and* inspects messages, drops RA and DHCP messages by default. **(HOST PORTS)**
...
-   **ND inspection**
    ...
    -   Drops messages from hosts that already exist in the bind table but on different hardware addresses, preventing address spoofing
...
-   **Source Guard**
    ...
    -   Snooping tool for validating source of IPv6 traffic - if traffic arriving is from an unknown source it can be blocked & dropped
        -   Source IP must be in the **IPv6 Binding** **table**.
